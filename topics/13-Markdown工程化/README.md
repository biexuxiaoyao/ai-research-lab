...
| IntelliJ LSP → Go to Def | Marksman LSP → Go to Def |
| SpotBugs/PMD → 静态分析 | markdownlint / vale → 规则检查 |
| grep / ast-grep → 结构化搜索 | ast-grep → "所有 ⬜ 状态的 L3 节点" |
| git diff → 行级 Diff | SemShift → 语义级 Diff |
| semantic-release → 版本发布 | DOA → 过时文档检测 |

```mermaid
mindmap
  root((Markdown 工程化<br/>工具栈全景))
    AST 与解析
