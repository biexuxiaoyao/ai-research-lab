...
- [第 07 章：CI/CD与DevOps](../../topics/07-CICD与DevOps/README.md) — CI 流水线安全扫描是纵深防御的第一道自动化门禁：SAST/SCA/密钥扫描在 PR 阶段的集成、SBOM 自动生成与签名验证，以及 AI 生成代码应视为"不受信任的第三方输入"的流水线策略
- [第 14 章：Agent Harness 与运行时](../../topics/14-Agent-Harness与运行时/README.md) — 安全工程的多层防御策略（10.2 提示注入防御、10.5 零信任架构）必须在 Harness 层落地：权限分级、沙盒隔离和工具调用审计是防止 Agent 被劫持的最后防线（参见 14.3 权限模型与沙盒）
